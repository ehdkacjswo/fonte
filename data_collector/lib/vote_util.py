...
# Load commit history with updated depth
def load_commit_history_org(fault_dir, tool, stage2):
    commit_df = pd.read_pickle(os.path.join(fault_dir, tool, "commits.pkl"))
    if "class_file" not in commit_df.columns:
        with open(os.path.join(fault_dir, "src_dir"), "r") as f:
            src_dir = f.read().strip()
            if src_dir[-1] != "/":
                src_dir += "/"
        commit_df["class_file"] = commit_df["src_path"].apply(
            lambda s: s[len(src_dir):]
        )
    commit_df["commit_hash"] = commit_df["commit_hash"].apply(lambda s: str(s)[:7])

    # Update depth
    excluded = get_style_change_commits(fault_dir, tool, stage2)
...
def vote_for_commits_org(fault_dir, tool, formula, decay, voting_func, \
    stage2, adjust_depth=True, in_class_only=False):
    commit_df = load_commit_history_org(fault_dir, tool, stage2)
...
# Get all style changes (commit, before path, after path)
...
# Load commit history with updated depth
def load_commit_history_new(fault_dir, tool, stage2):
    commit_df = pd.read_pickle(os.path.join(fault_dir, tool, "commits.pkl"))
    if "class_file" not in commit_df.columns:
        with open(os.path.join(fault_dir, "src_dir"), "r") as f:
            src_dir = f.read().strip()
    ...
    # Update depth
    excluded = get_style_changes(fault_dir, tool, stage2)
    commit_df["new_depth"] = commit_df["depth"]
...
def vote_for_commits_new(fault_dir, tool, formula, decay, voting_func, \
    stage2, adjust_depth=True, in_class_only=False):
    commit_df = load_commit_history_new(fault_dir, tool, stage2)
