...
independent_vars <- c("HSFL", "stage2", "score_mode", "ensemble", "use_diff", "use_stopword", "adddel")
selected_iv_combinations <- unlist(
  lapply(7:7, function(k) combn(independent_vars, k, simplify = FALSE)),
  recursive = FALSE
)
...
# Filter the data
data <- data %>% filter(HSFL %in% c("False", "True"))
...
final_results <- map(selected_dependent_vars, function(dv) {
  output_file <- file.path(output_dir, paste0("HSFL_results_", dv, ".csv"))
