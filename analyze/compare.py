...
# Fonte only + extra only 어떻게 처리?
# result_key = (str(HSFL), str(score_mode), str(ensemble), str(use_br), str(use_diff), str(stage2), str(use_stopword), str(adddel))
# original : result_key = (str(HSFL), 'None', str(ensemble), 'None', 'None', str(stage2), 'None', 'None')
# extra : ('None', str(score_mode), str(ensemble), str(use_br), str(use_diff), str(stage2), str(use_stopword), str(adddel))
def compare_num_iters():
    ...
    option_dict = {
        ...
        'stage2' : ['skip', 'True', 'False'],
        'use_stopword' : ['True', 'False'],
        'adddel' : ['add', 'all-uni', 'all-sep']
    }

    """option_dict = {
        'HSFL' : ['True', 'False'],
        'score_mode' : ['score', 'rank', 'both'],
        'ensemble' : ['mul', '(\'add\', 0.0)', '(\'add\', 0.1)', '(\'add\', 0.2)', '(\'add\', 0.3)', \
        '(\'add\', 0.4)',  '(\'add\', 0.5)', '(\'add\', 0.6)', '(\'add\', 0.7)', '(\'add\', 0.8)', \
        '(\'add\', 0.9)', '(\'add\', 1.0)'],
        'use_br' : ['False', 'True'],
        'use_diff' : ['True', 'False'],
    ...
    setting_list = list(itertools.product(option_dict['HSFL'], option_dict['score_mode'], option_dict['ensemble'], \
        option_dict['use_br'], option_dict['use_diff'], option_dict['stage2'], option_dict['use_stopword'], option_dict['adddel']))
    
    # Using only original fonte score
    setting_list_org = list(itertools.product(option_dict['HSFL'], ['None'], ['(\'add\', 1.0)'], \
        ['None'], ['None'], option_dict['stage2'], ['None'], ['None']))
    
    # Using only extra score
    setting_list_extra = list(itertools.product(['None'], ['(\'add\', 0.0)'], option_dict['ensemble'], \
        option_dict['use_br'], option_dict['use_diff'], option_dict['stage2'], option_dict['use_stopword'], option_dict['adddel']))
    
    # Compare the options of parameters
    for ind, param in enumerate(['HSFL', 'score_mode', 'ensemble', 'use_br', 'use_diff', 'stage2', 'use_stopword', 'adddel']):
        print(f'Working on {param}')
        option_num_iters_dict = dict()
        param_option_list = option_dict[param]
        ...
        for setting in setting_list: # For every possible setting
            for option in param_option_list: # For possible options for selected parameter
                if setting[ind] == option: # Gather the number of iterations
                    option_num_iters_dict[option] = option_num_iters_dict.get(option, []) + \
                        [num_iter[0] for num_iter in num_iters_dict[setting]]
        ...
        for ind1 in range(len(param_option_list)):
            for ind2 in range(ind1 + 1, len(param_option_list)):
                ...
                try:
                    pvalue = kruskal(option_num_iters_dict[option1], option_num_iters_dict[option2])
                except:
                    pvalue = 'Identical'

                print(f'Comparing {option1} and {option2}')
                print(f'Mean of num_iters {mean(option_num_iters_dict[option1])} {mean(option_num_iters_dict[option2])}')
                print(f'P-value : {pvalue}')

def best_setting_iter():
    with open('/root/workspace/num_iters.pkl', 'rb') as file:
        num_iters_dict = pickle.load(file)
    
    num_iters_list = []

    for setting, _num_iters_list in num_iters_dict.items():
        # Settings that are possible to make weighted bisection anavailable
        if (setting[1] == 'score' or setting[1] == 'both') and \
            (setting[2] == 'mul' or setting[2] == '(\'add\', 0.0)'):
            continue
        
        num_iters_list.append((setting, [data[0] for data in _num_iters_list]))

    num_iters_list.sort(key=lambda x : mean(x[1])) # Rank the settings
    best_setting = num_iters_list[0][0]
    best_num_iters = num_iters_list[0][1]
    print(f'Best setting : {best_setting}\nMean number of iterations : {mean(best_num_iters)}')
    
    best_param_set = set([0, 1, 2, 3, 4, 5, 6, 7])

    for (setting, num_iters) in num_iters_list[1:]:
        if len(num_iters) != len(best_num_iters):
            print(len(best_num_iters), len(num_iters))
        pvalue = wilcoxon(best_num_iters, num_iters, alternative='less').pvalue
        
        if pvalue > 0.05:
            for i in range(8):
                if best_setting[i] != setting[i]:
                    best_param_set.discard(i)
