...
def load_feature_data(pid, vid):
    ...
    def get_test_code(test_path, test_name, test_method):
        ...
        try:
            ...
            print(f'[ERROR] Failed to read test file {test_src_path}')
            return None, None
        ...
        if p.returncode != 0: # Parsing error
            print(f'[ERROR] Failed to parse test file {test_src_path}', out_txt, err_txt)
            return None, None

        try:
            parse_json = json.loads(out_txt.decode(encoding='utf-8', errors='ignore'))
        except:
            print(f'[ERROR] Failed to decode parsed test file {test_src_path}')
            return None, None
        ...
        if len(method_intvl_list) == 0:
            ...
            for extension in extension_list:
                ...
                # Multiple test found
                    ...
                    print(f'[ERROR] Multiple test method found {test_src_path}::{test_method}')
                    return None, None
    ...
    if p.returncode != 0:
        print(f'[ERROR] Checkout failed', out_txt, err_txt)
    ...
    try:
        ...
        print('[ERROR] Moving directory failed')
        return None, None, None
    ...
    if p.returncode != 0:
        print('[ERROR] Exporting test directory failed', main_test_path, err_txt)
        return None, None, None
